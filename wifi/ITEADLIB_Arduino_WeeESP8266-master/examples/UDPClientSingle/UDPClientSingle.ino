...
void loop(void)
{
    ...
    if (wifi.registerUDP(HOST_NAME, HOST_PORT)) {
        Serial.print("loop start :register udp ok\r\n");
    } else {
        Serial.print("loop start :register udp err\r\n");
    }
    
    char *hello = (char*)"Hello marc, this is client!";
    ...
    if (wifi.unregisterUDP()) {
        Serial.print("loop end :unregister udp ok\r\n");
    } else {
        Serial.print("loop end : unregister udp err\r\n");
    }
    delay(5000);
}
