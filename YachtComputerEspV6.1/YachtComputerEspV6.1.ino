...
void setup() {
  ...
  myLogger= new Logger(ptUSB,machineName,D);
  ...
  asyncrInputSer1=new AsyncrSerial(ptInputSer1,myLogger);
  ...
  const float refVoltage = 2519;      // in mV
  const float refVoltage2 = 2519;    // in mV !!!!!!!!!!!!!!!!!!!

  // const int refSource=DEFAULT;
  mInput = new SensorAnalogGroupInput(ptEspSer, myLogger, 'A',INT, pinNumber, scale, offset, refVoltage, EXTERNAL);
  mInput2 = new SensorAnalogGroupInput(ptEspSer, myLogger, 'A',VOL, pinNumber2, scale2, offset2, refVoltage2,DEFAULT);
#endif
  ...
  mySensor2 = new SensorHMC5883(ptEspSer, myLogger,DECLINAISON_DEG, DECLINAISON_MIN, X_OFFSET, Y_OFFSET, Z_OFFSET,SPEED_COEF, 12345);
    ...
    do{
      ...
      if (ptMgWifiIn!=NULL){
        ...
        if (ptMgWifiIn->indexOf(F("Broadcasting")) >=0) {
          ...
          _LOG_PRINT(M,F("setup esp received "),*ptMgWifiIn);          
        }
      }
...
void loop() {
  ...
  //--------------------------------------------------------
  // reception
 String * ptMsg=&String("No message");
...
/* 
    ...
    if (mySensor2 != NULL) {       
     ...
     // ptUSB->println(F("mySensor2 = NULL"));
...
*/
  ...
  if (nextTimeAnalogGroupInput + TIME_INTERVAL_ANALOG_GROUP < millis()) {
   ...
   // mInput->toNmea();
    mInput2->toNmea();
    _LOG_PRINT(D,F(" ANALOG_GROUP" ),F("end" ));
