...
_LOG_PRINT(M, (char*)"setup"," Initialise the sensor");
 /* mySensor = new SENSOR( ptLogOutputSer,
                                myLogger,
                                DECLINAISON_DEG,
                                DECLINAISON_MIN,
                                12345,
  ...
  if (mySensor==NULL)  {
    ...
    if (mySensor->getError()) {
      ...
      _LOG_PRINT(M, (char*)"Error ouverture SensorQMC5883  ", mySensor->getErrorMsg());
      delete mySensor;
      mySensor = NULL;
    } 
    else {
  ...
  if (mySensor != NULL) {
   ...
   // delay(100);
    if (lastTime + TIME_INTERVAL_GIROUETTE < millis()) {
      ...
      sendMsg(cmsg);
      _LOG_PRINT(W, "msg sendet", mySensor->toString());
     // ptLogOutputSer->print(mySensor->toString());
     if (mySensor == NULL) {
      _LOG_PRINT(V, "girouette  fin ", "mySensor = NULL");
     }
    }
  }
  else {
